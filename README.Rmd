...
We have developed two different methods for estimating cycle to rail stations. Both methods are focused on commuting. This means we are only considering the cycle-to-station potential for commuter journeys. People also travel by rail for various other purposes, such as leisure, business trips, education, and other personal business. (add in some stats from the ORR) Therefore our estimates do not capture the full potential for cycle to rail stations.

Both of our methods follow these steps:

1) Use 2011 Census data to find journeys to work that are made by rail in Bedfordshire. In the 2011 census, the main journey mode is recorded - defined as the mode of travel used for the single longest journey leg by distance - but secondary modes are not, which means we do not know which mode of transport rail commuters use to travel to the station. In addition to this, we do not have any information identifying which stations these commuters use.

2) Using our Phase 1 or Phase 2 approach, we identify which outbound rail station we expect the journey is likely to use, and map the route from home to this station using the Cyclestreets API. 

3) Having identified the expected routes to the station for qualifying Bedfordshire commuter journeys, the final step is to estimate the potential for the journey from home to the station to be cycled. For this, we use the Go Dutch scenario developed as part of the Propensity to Cycle Tool (Lovelace et al. 2017). This estimates what the cycle uptake would be if people in the UK cycled as much as they do in the Netherlands, taking into account hilliness and journey distance. 

## Phase 1 and Phase 2 approaches for journey routing

Both of our methods make use of 2011 Census data on travel to work. In reality the starting point we use for these journeys is not the commuter's home. Census data is anonymised and spatially aggregated at the LSOA (Lower Layer Super Output Area) level, a geographic unit with a population of around 1500. For each LSOA it is possible to calculate a 'centroid' marking the centre of gravity of the population of the LSOA. Thus, with the identities of the LSOAs each commuter lives and works in, we can use the LSOA centroids as the points of origin and destination for each journey.
...
Another minor limitation is related to the way in which we assess cycle routes to stations in Phase 2. To do this, we first use the CycleStreets API to identify routes from a given origin to nearby stations. This is based on a 'fastest route' algorithm which accounts for the hilliness of the route, delay at traffic signals, and road surface quality, as well as distance. This means the selected route to a particular station may not necessarily be the route which is shortest in terms of distance. However, once we have obtained these on-road routes from the origin to all nearby rail stations, we must then identify which station the commuter is assumed to be using. We do this based purely on the on-road distance from the journey origin to the station. It may be more appropriate to use journey time instead of distance to make this selection. 
...
In Phase 2 we exclude journeys to/from rail stations outside Bedfordshire, and journeys originating from outside the county to Bedfordeshire stations. These could be added in future, to better characterise journeys for residents living on the borders of the county.
