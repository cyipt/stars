...
```{r combi5, out.width="50%", fig.cap="The proportion of commuter journeys within Bedfordshire that are made using active travel, according to 2011 Census data"}
knitr::include_graphics("./figures/bedford-intra-region-commutes.png")
```

# Methodology

We have developed two different methods for estimating cycle to rail stations. Both methods are based on travel-to-work data from the 2011 Census. This means we are only considering the cycle-to-station potential for commuter journeys. People also travel by rail for various other purposes, such as leisure, business trips, education, and other personal business. According to the National Travel Survey 2018, 54% of of rail journeys are for commuting to work and school, while 26% are for leisure and 5% are for shopping (DfT 2019). Therefore our estimates do not capture the full potential for cycle to rail stations.
...
knitr::kable(totals,caption = "Total rail commuter journeys by Bedfordshire residents")
```

The total number of rail commuter journeys by Bedfordshire residents are shown in @ref/table1, showing only journeys that originate in Bedfordshire to allow comparability between the Phase 1 and Phase 2 methods. We can see that in the Phase 1 analysis, fewer rail journeys are modelled than the actual number of rail commutes according to 2011 Census data. Including all rail stations, only 80% of rail journeys are accounted for, based on 2011 Census returns. This suggests that for many current rail commutes in Bedfordshire, it is either impossible to reach the station by public transport or the three fastest journey options according to TransportAPI do not involve travel by rail. It is likely that in the majority of cases these commuters are driving to the station.
...
```{r table3}
totals_dutch = read.csv("../stars-data/data/flow/totals_dutch.csv")
knitr::kable(totals_dutch,caption = "Go Dutch cycle-to-station potential for Bedfordshire residents")
```
...
```{r table4}
s_counts_dutch = read.csv("../stars-data/data/flow/s_counts_dutch.csv")
knitr::kable(s_counts_dutch,caption = "Go Dutch cycle-to-station potential for rail journeys departing from each mainline station")
```

When we look at each station individually, we again find a similar disparity between the Phase 1 and Phase 2 approaches. In Phase 1, takeup reaches a maximum of 23% at Flitwick and a minimum of 11% at Luton Airport Parkway (excluding Harlington which was missed from the analysis), while in Phase 2 takeup ranges from 38% to 43% (Table 4). The low yet varying takeup of cycling under Phase 1 is due to the greater distances commuters are modelled as travelling to reach the station. Conversely, in Phase 2 all journeys to the station are relatively short, with the greatest on-road distance of x km corresponding with a maximum as-the-crow-flies distance of 5 km. This results in a high and relatively consistent rate of cycling takeup.

These results are visualised in @ref/combi8. Here we can see total rail commutes departing from each mainline station as estimated in Phase 1 and Phase 2, alongside the associated Go Dutch cycle-to-station potentials.
...
```{r combi9, out.width="100%", fig.cap="Existing cycle infrastructure in central Luton, as recorded by CyIPT"}
...
In Bedford the cycle provision is more heavily developed, as we can see in @ref/combi11. However, there appears to be a lack of cycle infrastructure in the immediate vicinity of Bedford Midland station. 

```{r combi11, out.width="100%", fig.cap="Go Dutch cycle-to-station potential: Bedford closeup"}
...
```{r combi12, out.width="33%", fig.show='hold', fig.cap="Go Dutch cycle-to-station potential: Central Bedfordshire towns. (a) Leighton Buzzard; (b) Flitwick and Harlington; (c) Arlesey, Biggleswade and Sandy."}
...
In Table 5 we can see the existing number of cycle racks at each station (using data from www.nationalrail.co.uk) and the potential number of cycle journeys to the station under a Go Dutch scenario in Phase 1 and Phase 2. Luton station looks to have a particular need for more cycle stands as cycling numbers increase. We can also see large number of cycle journeys predicted to Leagrave and Bedford stations.

```{r table5}
racks = read.csv("./output-data/racks.csv")
knitr::kable(racks,caption = "Existing cycle rack numbers and cycle-to-station potentials in Bedfordshire")
```
