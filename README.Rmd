...
There are several differences between the Phase 1 and Phase 2 approaches. The most important difference is in how we determine which rail stations are used. In Phase 1, the aim is to minimise the rail journey time, taking into account the final destination. In Phase 2 the aim is to minimise the distance from the home to the station, with no consideration given to the destination.
...
A further difference is that the Phase 2 approach focuses strictly on journeys that were made by rail in 2011. The Phase 1 approach models all commuter journeys that could potentially be made by rail. It does this by identifying whether the journey can be made by public transport (including rail, bus and walking) to allow arrival by 9:00 AM. Then we explore how to replace the non-rail portions of the journey with cycling.

In Phase 1, journeys with either an origin or destination in Bedfordshire are modelled, but in Phase 2 only journeys with an origin in Bedfordshire (i.e. Bedfordshire residents) are modelled. 

Finally, in Phase 2 we exclude journeys where the distance from the origin to the nearest station is greater than 5km as the crow flies. While this will capture the majority of cycleable trips, it would also be possible in future to increase this maximum distance to include longer journeys.
...
```{r table1}
totals = read.csv("../stars-data/data/flow/totals.csv")
knitr::kable(totals,caption = "Rail commuter journeys by Bedfordshire residents")
```

TX shows the number of rail journeys for each of the ten mainline stations under this scenario. 
Include table of total phase 1 (10 stations), total phase 1 (all stations), total phase 2, total census.

From table X, we can see that in the Phase 1 analysis, fewer rail journeys are modelled than the actual number of rail commutes according to 2011 Census data. Including all rail stations, only 80% of rail journeys are accounted for, while looking solely at journeys from the ten mainline Bedfordshire stations, only 70% of Bedfordshire rail journeys are accounted for, based on 2011 Census returns. Given that under the Phase 1 method, all journeys that can be undertaken by public transport are considered, this suggests that for many current rail commutes in Bedfordshire, it is impossible to reach the station by public transport. It is likely that in the majority of cases these commuters are driving to the station.

In Phase 2, only journeys starting within 5 km of a mainline rail station are included. This means that x% of rail journeys are missed, although these are the longer journeys which may be less likely to be cycled.

We can also see some notable differences in terms of which stations are being used. In particular, Phase 2 shows many more departures from Leagrave station. Most of these were routed to Luton station in the Phase 1 method. If we were focusing on travel to the station by car, bus or other modes, then it may be the case that most of these journeys should be routed to Luton station. However, our aim is to model potential cycle flows. Since both stations are on the same line, and both have a good quality rail service to London, with trains running at least every 15 minutes, it seems reasonable to assume that commuters who cycle to the station would simply cycle to their nearest station, and therefore would be likely to use Leagrave as long as it is their nearest station. 

In Phase 2 there are more cyclists at Arlesey and Sandy. In reality, these are probably people who live close to the county boundary and who actually use stations outside Bedfordshire, such as Hitchin and St Neots. 
...
Table X shows the number of cyclists entering each of the ten mainline stations under this scenario. 
Include table of total phase 1 (10 stations), total phase 1 (all stations), total phase 2.

There is a major difference between these two approaches in terms of the total number of cyclists. In Phase 1, there are x, while in Phase 2 there are three times that with x.

Again, there are differences in terms of which station cyclists are using. Leagrave....etc. 
...
In Phase 2 we exclude the possibility that Bedfordshire residents could use rail stations outside Bedfordshire. Stations within a 5 km buffer zone of the county could be added in future, to better characterise journeys for residents living on the borders of the county. 

It would also be possible in future to add in to the Phase 2 analysis journeys with a destination in Bedfordshire.

The most significant future improvement is to combine this cycle-to-stations analysis with analysis of the potential for direct non-rail cycle commuting, and with the potential for cycling to schools, both of which have been developed as part of the Propensity to Cycle Tool (Lovelace et al. 2017; Goodman et al. 2019). This brings us much closer to an estimation of total, multipurpose cycle flows. In combination, this will provide greatly improved estimates of cycle propensity across Bedfordshire, producing the best possible evidence supporting cycle infrastructure improvements in the county. 
