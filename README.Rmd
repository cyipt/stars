...
There are several differences between the Phase 1 and Phase 2 approaches. The most important difference is in how we determine which rail stations are used. In Phase 1, the aim is to minimise the rail journey time, while in Phase 2 the aim is to minimise the distance from the home to the station. 
...
Which method produces more rail journeys? Add table. If Phase 2, is it because many people are (driving to the station and) doing commutes for which there no is possible way to complete the journey by 9:00 AM by public transport?
